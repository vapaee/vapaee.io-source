...
export class DexTokenPage implements OnInit, OnDestroy, AfterViewInit {

    token: TokenDEX = new TokenDEX();
    editing: boolean = false;
    newadmin: boolean = false;
    adminname: string = "";
    feed: Feedback = new Feedback();
    events: string[] = [];
    editevent: TokenEvent | null = null;
    showactions: boolean = false;
    action: string = "";
    params: {quantity?:AssetDEX|null, from?:string|null} = {};
    _safe_url_cache:any = {};
    ...
    constructor(
        ...
        this.dex.waitTokenData.then(_ => {
            ...
            console.log("DexTokenPage() token: ", [this.token]);
    ...
    fetchTwitter(info: TokenData) {
        ...
        if (clean_link) {
            let url = "https://publish.twitter.com/oembed?url="+clean_link[1];
    ...
    renderYoutube(info: TokenData, delay:number) {
        setTimeout(() => {
            ...
            if (gapi) {                
                ...
                gapi.load("", {
                    callback: (<any>window).gapi_onload,
                    _c: {
                        "jsl": {
                            "ci": {
                                "deviceType": "desktop",
    ...
    setAction(act:string) {
        switch (act) {
            case "transfer":
                this.params.from = this.dex.account_name;
                break;
            case "issue":
            case "open":
                break;
    ...
    tryToExtractChannelId(info:TokenData) {
        ...
        if(info.category == "youtube") {            
            ...
            let result = info.link.match(/channel\/(.*)$/);
            if (result) {
                channel_id = result[1];
    ...
    tryToExtractVideoLink(info:TokenData) {
        ...
        if(info.category == "youtube") {
            ...
            if (result && !finalsrc) {
                ...
                if (finalsrc) this._safe_url_cache[info_id] = finalsrc;
    ...
    get exploreUrl() {
        if (this.token.ok) return "https://telos.eosx.io/account/" + this.token.contract;
        return null;
    }
    get actionsUrl() {
        if (this.token.ok) return "https://telos.eosx.io/tools/contract?contractAccount=" + this.token.contract;
        return null;
    }
    ...
    get lgicon() {
        ...
        if (this.token.iconlg) return this.token.iconlg;
    ...
    get hide_edit_btn(): boolean {
        if (this.dex.logged) {
            if (!this.editing) {
                if (this.token.ok) {
                    if (this.token.admin == this.dex.account_name) {
                        // console.log("DexTokenPage.hide_edit_btn() -> false");
                        return false;
                    }
                }
            }
        }
        // console.log("DexTokenPage.hide_edit_btn() -> true");
        return true;
    }
    ...
    editToken() {
        console.log("DexTokenPage.editToken()");
        this.editing = true;
    }

    quitEditing() {
        console.log("DexTokenPage.quitEditing()");
        this.editing = false;
        this.renderEntries();
    }

    onTokenInfoChange() {
        console.log("DexTokenPage.onTokenInfoChange()");
    }
    ...
    confirm() {
        ...
        this.token.iconlg = this.token.iconlg || this.token.icon;
    ...
    changeAdmin() {
        ...
        this.dex.tokenadmin(this.token, this.adminname).then(_ => {
            ...
            this.renderEntries();
            if (this.token.ok) this.token.admin = this.adminname;
        }).catch(e => {
            console.error(e);
            this.feed.setLoading("new-admin", false);
    ...
    confirmData(info:TokenData) {
        console.log("DexTokenPage.confirmData()", [info]);
        if(!this.token.ok) return;
        let action = "modify";
    ...
    removeData(info:TokenData) {
        console.log("DexTokenPage.removeData()", [info]);
        let action = "remove";
        if(!this.token.ok) return;
        info.symbol = this.token.symbol;
        this.feed.setLoading("info-" + info.id, true);
        ...
        this.dex.settokeninfo(action, info).then(_ => {
            console.log("EXITO:", _);
            if(!this.token.ok) return;
            for (let i=0; i<this.token.data.length; i++) {
    ...
    createDataEntry() {
        console.log("DexTokenPage.createDataEntry()");
        if(!this.token.ok) return;
        let entry: TokenData = {
            id:-1,
            category: "youtube",
        ...
        setTimeout(() => {
            entry.editing = true;
            let element:HTMLElement | null = window.document.getElementById("input-1");
    ...
    editTokenEvent(evt:TokenEvent) {
        ...
        setTimeout(() => {
            let element:HTMLElement | null = window.document.getElementById("new-event");
            if (element) element.focus();
        }, 100);        
    }

    cancelTokenEvent() {
        console.log("DexTokenPage.cancelTokenEvent()", [this.editevent]);
        if (this.editevent && this.token) {
            if (this.editevent.new) {
                for (let i=0; i<this.token.events.length; i++) {
                    if (this.token.events[i].event == this.editevent.event) {
                        this.token.events.splice(i, 1);
                    }
                }    
            }
            this.editevent.editing = false;    
        }
        this.editevent = null;
    }

    confirmEvent(evt:TokenEvent) {
        console.log("DexTokenPage.confirmEvent()", [evt]);
        if(!this.token.ok) return;
        let action = "modify";
        if (evt.new) action = "add";
        let feedid = "event-save";
    ...
    removeEvent(evt:TokenEvent) {
        console.log("DexTokenPage.removeEvent()", [evt]);
        if(!this.token.ok) return;
        let feedid = "event-remove";
        this.feed.setLoading(feedid, true);
        this.feed.clearError("event");
    ...
    createTokenEvent() {
        console.log("DexTokenPage.createTokenEvent()");
        if(!this.token.ok) return;
        this.cancelTokenEvent();
        this.events = ["cancel", "deal", "deposit", "order", "swapdeposit", "withdraw"];
        for (let i in this.token.events) {
            let event = this.token.events[i].event;
